...
<style>
  ...
  /* ── 상단 툴바 아이콘 숨기기 (Share, Star, Edit, GitHub 등) ── */
...
def cpk_level(cpk_val):
    """Cpk 값으로 공정능력수준 반환 → (레벨명, 색상코드)"""
    if cpk_val is None:
        return "—", "#9ca3af"
    if cpk_val >= 1.67:
        return "공정능력 충분",      "#059669"   # 진초록
    elif cpk_val >= 1.33:
        return "공정능력 다소충분",  "#16a34a"   # 초록
    elif cpk_val >= 1.00:
        return "공정능력 보통",      "#d97706"   # 주황
    elif cpk_val >= 0.67:
        return "공정능력 부족",      "#ea580c"   # 진주황
    else:
        return "공정능력 매우부족",  "#dc2626"   # 빨강
...
def build_report_html(mat, results, stats, prev, current_user):
    ...
    if stats:
        ...
        stat_items = (
            ...
            si('Cpk', f"{s['cpk']:.3f}" if s['cpk'] else '—', cpk_level(s['cpk'])[1]) +
            si('공정능력', cpk_level(s['cpk'])[0], cpk_level(s['cpk'])[1])
        )
    ...
    with left:
        ...
        if stats:
            ...
            if s['cpk'] is not None:
                lvl_name, lvl_color = cpk_level(s['cpk'])
                st.markdown(
                    f"<div style='margin-top:8px; padding:7px 10px; border-radius:6px;"
                    f"background:{lvl_color}18; border:1px solid {lvl_color}55;'>"
                    f"<span style='font-size:10px;color:#6b7280'>참고사항:&nbsp;</span>"
                    f"<span style='font-size:12px;font-weight:700;color:{lvl_color}'>{lvl_name}</span>"
                    f"</div>",
                    unsafe_allow_html=True)
        else:
...
def page_audit():
    ...
    fc1, fc2, fc3, fc4, fc5 = st.columns([1.3, 1.3, 1.7, 1.7, 0.7])
    with fc1:
        st.markdown("<label style='font-size:12px;color:#374151;font-weight:500'>날짜 시작</label>", unsafe_allow_html=True)
        d_from = st.date_input("날짜 시작", value=None, key="af", label_visibility="collapsed")
    with fc2:
        st.markdown("<label style='font-size:12px;color:#374151;font-weight:500'>날짜 종료</label>", unsafe_allow_html=True)
        d_to   = st.date_input("날짜 종료", value=None, key="at", label_visibility="collapsed")
    with fc3:
        st.markdown("<label style='font-size:12px;color:#374151;font-weight:500'>아이디 / 이름 검색</label>", unsafe_allow_html=True)
        u_flt  = st.text_input("사용자", placeholder="검색...", label_visibility="collapsed", key="au")
    with fc4:
        st.markdown("<label style='font-size:12px;color:#374151;font-weight:500'>작업 유형 검색</label>", unsafe_allow_html=True)
        a_flt  = st.text_input("작업",   placeholder="검색...", label_visibility="collapsed", key="aa")
    with fc5:
        st.markdown("<label style='font-size:12px;color:#374151;font-weight:500'>&nbsp;</label>", unsafe_allow_html=True)
        if st.button("초기화", key="a_reset", use_container_width=True):
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: Data Management (수치 관리) — Admin Only
# ═══════════════════════════════════════════════════════════════
def page_data_mgmt():
    if st.session_state.user['role'] != 'admin':
        st.error("관리자 권한이 필요합니다.")
    ...
    dm_tab1, dm_tab2, dm_tab3 = st.tabs(["📦 원료 관리", "📐 Spec (USL/LSL) 설정", "🔢 시험 데이터 관리"])

    # ══════════════════════════════════════
    #  서브탭 1: 원료 추가 / 수정 / 삭제
    # ══════════════════════════════════════
    with dm_tab1:
        st.markdown("#### 📦 원료 목록 관리")
        materials = get_materials()

        # ── 원료 테이블 헤더
        hcols = st.columns([1.2, 3.5, 1.2, 1.2, 0.9, 0.9])
        for col, txt in zip(hcols, ["원료코드", "원료명", "USL", "LSL", "수정", "삭제"]):
            col.markdown(
                f"<div style='font-size:11px;font-weight:600;color:#6b7280;"
                f"padding:4px 2px;border-bottom:2px solid #e2e8f0'>{txt}</div>",
                unsafe_allow_html=True)

        for m in materials:
            row = st.columns([1.2, 3.5, 1.2, 1.2, 0.9, 0.9])
            row[0].markdown(f"<div style='padding:6px 2px;font-size:12px;font-family:monospace'>{m['code']}</div>", unsafe_allow_html=True)
            row[1].markdown(f"<div style='padding:6px 2px;font-size:12px'>{m['name']}</div>", unsafe_allow_html=True)
            row[2].markdown(f"<div style='padding:6px 2px;font-size:12px;color:#059669;font-weight:600'>{m['usl']}</div>", unsafe_allow_html=True)
            row[3].markdown(f"<div style='padding:6px 2px;font-size:12px;color:#059669;font-weight:600'>{m['lsl']}</div>", unsafe_allow_html=True)
            if row[4].button("✏️", key=f"dm_me_{m['code']}", help="수정"):
                st.session_state.dm_edit_mat = dict(m)
            if row[5].button("🗑️", key=f"dm_md_{m['code']}", help="삭제"):
                st.session_state.dm_del_mat = m['code']

        st.divider()

        # ── 삭제 확인
        if st.session_state.get('dm_del_mat'):
            code = st.session_state.dm_del_mat
            mat_name = next((m['name'] for m in materials if m['code'] == code), code)
            st.warning(f"⚠️ **{code} — {mat_name}** 를 삭제하시겠습니까? (연관된 시험 데이터도 모두 삭제됩니다)")
            dc1, dc2, _ = st.columns([1, 1, 6])
            with dc1:
                if st.button("확인 삭제", type="primary", key="dm_del_confirm"):
                    sb.table('test_results').delete().eq('material_code', code).execute()
                    sb.table('materials').delete().eq('code', code).execute()
                    log_audit("원료 삭제", f"코드:{code}, 이름:{mat_name}")
                    del st.session_state.dm_del_mat
                    st.success(f"{code} 삭제 완료")
                    st.rerun()
            with dc2:
                if st.button("취소", key="dm_del_cancel"):
                    del st.session_state.dm_del_mat
                    st.rerun()

        # ── 원료 수정 폼
        if st.session_state.get('dm_edit_mat'):
            em = st.session_state.dm_edit_mat
            st.markdown(f"#### ✏️ 원료 수정: {em['code']}")
            with st.form("dm_edit_mat_form"):
                e1, e2 = st.columns(2)
                with e1: new_code = st.text_input("원료코드", value=em['code'])
                with e2: new_name = st.text_input("원료명",   value=em['name'])
                ef1, ef2, _ = st.columns([1, 1, 4])
                with ef1: save   = st.form_submit_button("저장", type="primary")
                with ef2: cancel = st.form_submit_button("취소")
            if save:
                sb.table('materials').update(
                    {'code': new_code, 'name': new_name}
                ).eq('code', em['code']).execute()
                log_audit("원료 수정", f"코드:{em['code']}→{new_code}, 이름:{em['name']}→{new_name}")
                del st.session_state.dm_edit_mat
                st.success("수정 완료")
                st.rerun()
            if cancel:
                del st.session_state.dm_edit_mat
        ...
        # ── 원료 추가 폼
        st.markdown("#### ➕ 원료 추가")
        with st.form("dm_add_mat_form"):
            a1, a2, a3, a4 = st.columns([1.2, 3, 1, 1])
            with a1: add_code = st.text_input("원료코드", placeholder="1234567")
            with a2: add_name = st.text_input("원료명",   placeholder="Acetaminophen")
            with a3: add_usl  = st.number_input("USL", value=103.0, step=0.5, format="%.1f")
            with a4: add_lsl  = st.number_input("LSL", value=97.0,  step=0.5, format="%.1f")
            submitted = st.form_submit_button("➕ 추가", type="primary")
        if submitted:
            if not add_code.strip() or not add_name.strip():
                st.error("원료코드와 원료명을 입력하세요.")
            elif add_lsl >= add_usl:
                st.error("USL은 LSL보다 커야 합니다.")
            else:
                exists = sb.table('materials').select('code').eq('code', add_code.strip()).execute().data
                if exists:
                    st.error(f"원료코드 {add_code} 가 이미 존재합니다.")
                else:
                    ...
                    log_audit("원료 추가", f"코드:{add_code}, 이름:{add_name}, USL:{add_usl}, LSL:{add_lsl}")
                    st.success(f"{add_code} — {add_name} 추가 완료!")
                    st.rerun()

    # ══════════════════════════════════════
    #  서브탭 2: Spec (USL/LSL) 일괄 설정
    # ══════════════════════════════════════
    with dm_tab2:
        st.markdown("#### 📐 원료별 Spec (USL/LSL) 설정")
        materials = get_materials()

        # 헤더
        sh = st.columns([1.2, 3.2, 1.3, 1.3, 1.0])
        for col, txt in zip(sh, ["원료코드", "원료명", "USL", "LSL", "저장"]):
            col.markdown(
                f"<div style='font-size:11px;font-weight:600;color:#6b7280;"
                f"padding:4px 2px;border-bottom:2px solid #e2e8f0'>{txt}</div>",
                unsafe_allow_html=True)
        ...
        for m in materials:
            sc = st.columns([1.2, 3.2, 1.3, 1.3, 1.0])
            sc[0].markdown(f"<div style='padding:5px 2px;font-size:11px;font-family:monospace;color:#374151'>{m['code']}</div>", unsafe_allow_html=True)
            sc[1].markdown(f"<div style='padding:5px 2px;font-size:12px'>{m['name']}</div>", unsafe_allow_html=True)
            new_usl = sc[2].number_input("", value=float(m['usl']), step=0.5, format="%.1f",
                                          key=f"spec_usl_{m['code']}", label_visibility="collapsed")
            new_lsl = sc[3].number_input("", value=float(m['lsl']), step=0.5, format="%.1f",
                                          key=f"spec_lsl_{m['code']}", label_visibility="collapsed")
            if sc[4].button("💾", key=f"spec_save_{m['code']}", help="저장"):
                if new_lsl >= new_usl:
                    st.error(f"[{m['code']}] USL은 LSL보다 커야 합니다.")
                else:
                    sb.table('materials').update({'usl': new_usl, 'lsl': new_lsl}).eq('code', m['code']).execute()
                    log_audit("Spec 변경", f"[{m['code']}] USL:{m['usl']}→{new_usl}, LSL:{m['lsl']}→{new_lsl}")
                    st.success(f"{m['code']} Spec 저장 완료 (USL:{new_usl} / LSL:{new_lsl})")
                    st.rerun()
    ...
    # ══════════════════════════════════════
    with dm_tab3:
        st.markdown("#### 🔢 시험 데이터 관리")
        materials = get_materials()

        sel_code = st.selectbox(
            "원료 선택",
            options=[m['code'] for m in materials],
            format_func=lambda c: next((f"{c} — {m['name']}" for m in materials if m['code'] == c), c),
            key="dm_sel_mat"
        )
        mat = next((m for m in materials if m['code'] == sel_code), None)
        if not mat:
            st.info("원료를 선택하세요.")
            return

        results = get_results(sel_code)
        usl, lsl = float(mat['usl']), float(mat['lsl'])
        stats = calc_stats([r['value'] for r in results], usl, lsl)

        if not results:
            st.info("해당 원료의 시험 데이터가 없습니다.")
        else:
            # 테이블 헤더
            dh = st.columns([0.6, 1.8, 1.5, 1.5, 0.7, 0.7])
            for col, txt in zip(dh, ["No.", "성적번호", "결과값", "판정", "수정", "삭제"]):
                col.markdown(
                    f"<div style='font-size:11px;font-weight:600;color:#6b7280;"
                    f"padding:4px 2px;border-bottom:2px solid #e2e8f0'>{txt}</div>",
                    unsafe_allow_html=True)
            ...
            for i, r in enumerate(results):
                status = get_status(r['value'], stats, usl, lsl) if stats else '—'
                sc_color = {'Pass':'#059669','OOT':'#d97706','OOS':'#dc2626'}.get(status,'#374151')
                dr = st.columns([0.6, 1.8, 1.5, 1.5, 0.7, 0.7])
                dr[0].markdown(f"<div style='padding:6px 2px;font-size:12px;color:#9ca3af'>{i+1}</div>", unsafe_allow_html=True)
                dr[1].markdown(f"<div style='padding:6px 2px;font-size:12px;font-family:monospace'>{r['lot_no']}</div>", unsafe_allow_html=True)
                dr[2].markdown(f"<div style='padding:6px 2px;font-size:12px;font-weight:600'>{r['value']}</div>", unsafe_allow_html=True)
                dr[3].markdown(f"<div style='padding:6px 2px;font-size:12px;font-weight:700;color:{sc_color}'>{status}</div>", unsafe_allow_html=True)
                if dr[4].button("✏️", key=f"dm_re_{r['id']}", help="수정"):
                    st.session_state.dm_edit_result = dict(r)
                if dr[5].button("🗑️", key=f"dm_rd_{r['id']}", help="삭제"):
                    sb.table('test_results').delete().eq('id', r['id']).execute()
                    log_audit("데이터 삭제", f"[{sel_code}] LOT:{r['lot_no']}({r['value']}) 삭제")
                    st.success(f"LOT {r['lot_no']} 삭제 완료")
                    st.rerun()

        # ── 개별 결과 수정 폼
        if st.session_state.get('dm_edit_result'):
            er = st.session_state.dm_edit_result
            st.divider()
            st.markdown(f"#### ✏️ 시험 데이터 수정: {er['lot_no']}")
            with st.form("dm_edit_result_form"):
                rf1, rf2 = st.columns(2)
                with rf1: new_lot_no = st.text_input("성적번호", value=er['lot_no'])
                with rf2: new_value  = st.number_input("결과값", value=float(er['value']), step=0.01, format="%.3f")
                rc1, rc2, _ = st.columns([1, 1, 5])
                with rc1: rsave   = st.form_submit_button("저장", type="primary")
                with rc2: rcancel = st.form_submit_button("취소")
            if rsave:
                if not re.match(r'^\d{6}I$', new_lot_no.strip().upper(), re.IGNORECASE):
                    st.error("성적번호 형식: 숫자 6자리 + I")
                else:
                    sb.table('test_results').update({
                        'lot_no': new_lot_no.strip().upper(),
                        'value':  new_value
                    }).eq('id', er['id']).execute()
                    log_audit("데이터 수정",
                        f"[{sel_code}] {er['lot_no']}({er['value']}) → {new_lot_no}({new_value})")
                    del st.session_state.dm_edit_result
                    st.success("수정 완료")
                    st.rerun()
            if rcancel:
                del st.session_state.dm_edit_result
        ...
        # ── 수동 데이터 추가
        st.divider()
        st.markdown("#### ➕ 시험 데이터 직접 추가")
        with st.form("dm_add_result_form"):
            ra1, ra2, _ = st.columns([1.5, 1.2, 4])
            with ra1: add_lot = st.text_input("성적번호 (6자리+I)", placeholder="191234I")
            with ra2: add_val = st.number_input("결과값", step=0.01, format="%.3f", key="dm_add_val")
            add_submitted = st.form_submit_button("➕ 추가", type="primary")
        if add_submitted:
            lot_clean = add_lot.strip().upper()
            if not re.match(r'^\d{6}I$', lot_clean, re.IGNORECASE):
                st.error("성적번호 형식: 숫자 6자리 + I  (예: 191234I)")
            elif any(r['lot_no'].upper() == lot_clean for r in results):
                st.error("이미 존재하는 성적번호입니다.")
            else:
                new_list = results + [{'lot_no': lot_clean, 'value': add_val, 'material_code': sel_code}]
                new_list.sort(key=lambda x: parse_lot(x['lot_no']))
                if len(new_list) > 15:
                    to_del = new_list[:len(new_list)-15]
                    for rd in to_del:
                        if 'id' in rd:
                            sb.table('test_results').delete().eq('id', rd['id']).execute()
                sb.table('test_results').insert({
                    'material_code': sel_code,
                    'lot_no':        lot_clean,
                    'value':         add_val,
                    'created_at':    datetime.now().isoformat()
                }).execute()
                log_audit("데이터 추가(수동)", f"[{sel_code}] LOT:{lot_clean}, 값:{add_val}")
                st.success(f"LOT {lot_clean} 추가 완료!")
                st.rerun()
...
def main():
    ...
    # ── 탭 네비게이션 ─────────────────────────────────────────
    if user['role'] == 'admin':
        tabs = st.tabs(["📊 원료 분석", "🗂️ 수치 관리", "👥 사용자 관리", "📋 Audit Trail"])
        with tabs[0]: page_analysis()
        with tabs[1]: page_data_mgmt()
        with tabs[2]: page_users()
        with tabs[3]: page_audit()
    else:
        page_analysis()
